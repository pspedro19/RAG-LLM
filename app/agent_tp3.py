...
if __name__ == "__main__":
    async def main():
        print("🏝️ Asistente Turístico de Curazao (LangGraph + ReAct)")
        print("Con mejores prácticas implementadas para sistemas multi-agente")
        
        # Crear ID de conversación para toda la sesión
        ...
        while True:
            ...
            print("\n📊 Estadísticas:")
            print(f"Tiempo total: {result.get('total_time', 0):.2f} segundos")
            print(f"Tipo de consulta: {result.get('query_type', '')}")
            print(f"Agentes utilizados: {', '.join(result.get('active_agents', []))}")
            
            # Mostrar detalle de tokens
            if "tokens" in result:
                print("\n💰 Uso de tokens:")
                print(f"Tokens de entrada: {result['tokens'].get('prompt', 0)}")
                print(f"Tokens de salida: {result['tokens'].get('completion', 0)}")
                print(f"Tokens de razonamiento: {result['tokens'].get('reasoning', 0)}")
                print(f"Tokens totales: {result['tokens'].get('total', 0)}")
            
            # Mostrar pasos ReAct
            if "react_steps_count" in result:
                print(f"\nPasos de razonamiento ReAct: {result.get('react_steps_count', 0)}")
            
            # Mostrar advertencias si existen
            if result.get("warnings", []):
                print("\n⚠️ Advertencias:", ", ".join(result.get("warnings", [])))
