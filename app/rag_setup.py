...
class RAGSetup:
    ...
    async def clear_database_tables(self) -> Dict[str, Any]:
        """
        Limpia las tablas de embeddings en la base de datos.
        
        Returns:
            Diccionario con estado de la operación
        """
        logger.info("Limpiando tablas de la base de datos...")
        
        # Limpiar tablas a través del repositorio de embeddings
        stats_before = await self.embedding_repository.get_stats()
        await self.embedding_repository.clear_all_embeddings()
        stats_after = await self.embedding_repository.get_stats()
        
        # Si también hay un índice FAISS, limpiarlo
        faiss_info_before = self.faiss_store.get_index_info()
        self.faiss_store.clear_index()
        faiss_info_after = self.faiss_store.get_index_info()
        
        result = {
            "embeddings_before": stats_before.get("total", 0),
            "embeddings_after": stats_after.get("total", 0),
            "faiss_vectors_before": faiss_info_before.get("total_vectors", 0),
            "faiss_vectors_after": faiss_info_after.get("total_vectors", 0)
        }
        
        logger.info(f"Base de datos limpiada: {result['embeddings_before']} embeddings eliminados")
        return result
...
async def main():
    ...
    # Comando limpiar
    clear_parser = subparsers.add_parser('clean', help='Limpiar tablas de la base de datos')
    clear_parser.add_argument('--use-container-name', action='store_true',
                            help='Usar postgres_pgvector en lugar de localhost')
    ...
    try:
        if args.command == 'ingest':
            ...
            if not args.force:
                confirm = input("¿Está seguro de que desea ELIMINAR TODOS los datos? (escriba 'CONFIRMAR' para continuar): ")
                if confirm != "CONFIRMAR":
                    print("Operación cancelada.")
                    return
            
            # Limpiar tablas
            result = await setup.clear_database_tables()
            print(f"\nBase de datos limpiada: {result['embeddings_before']} embeddings eliminados")
...
async def run_wizard(setup: RAGSetup):
    ...
    while True:
        ...
        print("5. Limpiar tablas de la base de datos")
        print("0. Salir")
