...
from app.ingestion.document_processor import DocumentProcessor, DocumentInput
...
def test_document_from_app_folder(document_reader, document_processor, test_config):
    """Prueba la lectura de documentos desde la carpeta app/data/documents."""
    # Crear un archivo de texto en la carpeta de documentos de app
    test_file = test_config.KB_DIR / "txt" / "test_document.txt"
    os.makedirs(test_config.KB_DIR / "txt", exist_ok=True)
    
    content = "Este es un documento de prueba específico para app.\nVerifica que la lectura funciona correctamente."
    ...
    try:
        ...
        assert doc_input.title == "Documento de app"
    finally:
        # Limpiar: eliminar el archivo de prueba
        if os.path.exists(test_file):
            os.remove(test_file)

def test_pdf_documents_in_app(document_reader, test_config):
    """Verifica los documentos PDF en app/data/documents/pdf."""
