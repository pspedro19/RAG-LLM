...
class EmbeddingRepository:
    ...
    async def clear_all_embeddings(self):
        """Elimina todos los embeddings de la base de datos."""
        logger = logging.getLogger("app.embeddings.embedding_repository")
        logger.info("Eliminando todos los embeddings y documentos relacionados...")
        
        pool = await self._get_pool()
        async with pool.acquire() as conn:
            # Iniciar una transacción para asegurar la consistencia
            async with conn.transaction():
                # Eliminar de la tabla de embeddings
                await conn.execute("DELETE FROM embeddings")
                logger.info("Tabla de embeddings limpiada")
                
                # Eliminar de la tabla de chunks
                await conn.execute("DELETE FROM chunks")
                logger.info("Tabla de chunks limpiada")
                
                # Eliminar de la tabla de documentos
                await conn.execute("DELETE FROM documents")
                logger.info("Tabla de documentos limpiada")
                
        logger.info("Todas las tablas de la base de datos han sido limpiadas")
