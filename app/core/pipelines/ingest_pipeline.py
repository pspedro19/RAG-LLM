...
class DocumentIngestionPipeline:
    ...
    async def process_documents(self, doc_dir: str, limit: Optional[int] = None) -> List[str]:
        ...
        # Mejorado: Log de los archivos en el directorio para debugging
        logger.info(f"Contenido del directorio {doc_dir}:")
        if pdf_dir.exists():
            contents = list(pdf_dir.glob("*"))
            for item in contents:
                logger.info(f"  - {item} ({'Directorio' if item.is_dir() else 'Archivo'})")
        else:
            logger.error(f"El directorio {doc_dir} no existe")
            return []
        
        # MODIFICADO: Buscar PDFs directamente en el directorio principal
        logger.info("Buscando PDFs en el directorio principal...")
        for pdf_file in pdf_dir.glob("*.pdf"):
            category = pdf_dir.name
            documents.append((pdf_file, category))
            logger.info(f"Encontrado PDF en directorio principal: {pdf_file}")
        
        # También buscar en minúsculas y mayúsculas
        for pdf_file in pdf_dir.glob("*.PDF"):
            category = pdf_dir.name
            documents.append((pdf_file, category))
            logger.info(f"Encontrado PDF (mayúsculas) en directorio principal: {pdf_file}")
        
        # También buscar en subdirectorios como estaba antes
        logger.info("Buscando PDFs en subdirectorios...")
        for subdir in pdf_dir.glob("*"):
            if subdir.is_dir():
                logger.info(f"Revisando subdirectorio: {subdir}")
                for pdf_file in subdir.glob("*.pdf"):
                    documents.append((pdf_file, subdir.name))
                    logger.info(f"Encontrado PDF en subdirectorio: {pdf_file}")
                # También buscar en mayúsculas
                for pdf_file in subdir.glob("*.PDF"):
                    documents.append((pdf_file, subdir.name))
                    logger.info(f"Encontrado PDF (mayúsculas) en subdirectorio: {pdf_file}")
        
        if not documents:
            # Intento alternativo con os.walk para verificar
            logger.warning(f"No PDFs encontrados con Path.glob, intentando con os.walk...")
            for root, dirs, files in os.walk(doc_dir):
                for file in files:
                    if file.lower().endswith('.pdf'):
                        full_path = Path(os.path.join(root, file))
                        category = Path(root).name
                        documents.append((full_path, category))
                        logger.info(f"Encontrado PDF con os.walk: {full_path}")
